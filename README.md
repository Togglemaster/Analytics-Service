...
### Obrigatórias
- **`AWS_REGION`** - Região dos serviços AWS (ex: `us-east-1`)
- **`AWS_SQS_URL`** - URL completa da fila SQS
  - Exemplo: `https://sqs.us-east-1.amazonaws.com/123456789012/togglemaster-events`
- **`AWS_DYNAMODB_TABLE`** - Nome da tabela DynamoDB para armazenar analytics
  - Exemplo: `ToggleMasterAnalytics`

### Credenciais AWS (necessárias mas não definidas no .env)
- **`AWS_ACCESS_KEY_ID`** - Access Key ID da AWS
- **`AWS_SECRET_ACCESS_KEY`** - Secret Access Key da AWS  
- **`AWS_SESSION_TOKEN`** - Token de sessão (se usando AWS Academy ou role temporário)

### Outras Variáveis
- **`PORT`** - Porta onde o serviço irá rodar (padrão: `8005`)
...
As credenciais devem ser configuradas via:
1. **AWS CLI**: `aws configure`
2. **Variáveis de ambiente**:
   ```bash
   export AWS_ACCESS_KEY_ID="sua-access-key"
   export AWS_SECRET_ACCESS_KEY="sua-secret-key"
   export AWS_SESSION_TOKEN="seu-token"  # se necessário
   ```
3. **IAM Role** (se rodando em EC2/ECS)
